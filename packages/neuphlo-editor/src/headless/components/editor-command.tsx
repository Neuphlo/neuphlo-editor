...
export function useEditorCommand() {
  ...
  if (!ctx) throw new Error("useEditorCommand must be used within <EditorCommand>");
  return ctx;
}
...
export const EditorCommand = forwardRef<HTMLDivElement, EditorCommandProps>(
  ({ className, children, ...rest }, ref) => {
    ...
    const value = useMemo<Ctx>(() => ({ query, setQuery, range, setRange }), [query, range]);
    ...
    return (
      <EditorCommandContext.Provider value={value}>
        <div
          ...
          onKeyDown={(e) => {
            // prevent editor from hijacking navigation while menu is open
          ...
          {...rest}
        >
          {/* hidden input to mirror example API; not relied upon */}
          <input style={{ display: "none" }} value={query} onChange={(e) => setQuery(e.target.value)} />
          {children}
        </div>
...
export const EditorCommandList = forwardRef<HTMLUListElement, ComponentPropsWithoutRef<"ul">>(
  ({ className, children, ...rest }, ref) => (
    <ul ref={ref} role="listbox" className={["nph-command__list", className].filter(Boolean).join(" ")} {...rest}>
      {children}
    </ul>
  ),
);
...
// Item moved to its own file for parity with bubble item
